...
openai_create_edit <- function(model,
  ...
  assertthat::assert_that(
    ...
    assertthat::is.number(temperature) && value_between(temperature, 0, 2),
    assertthat::is.number(top_p),
    assertthat::is.string(openai_api_key),
    is.null(openai_organization) || assertthat::is.string(openai_organization),
    value_between(top_p, 0, 1)
  )

  if (both_specified(temperature, top_p)) {
    rlang::warn("It is recommended NOT to specify temperature and top_p at a time.")
  }
...
openai_create_completion <- function(model,
  ...
  assertthat::assert_that(
    ...
    assertthat::is.number(temperature) && value_between(temperature, 0, 2),
    assertthat::is.number(top_p) && value_between(top_p, 0, 1),
    assertthat::assert_that(assertthat::is.count(n)),
    assertthat::is.flag(echo),
    assertthat::is.number(frequency_penalty) && value_between(frequency_penalty, -2, 2),
    is.character(stop) && length_between(stop, 1, 4) || is.null(stop),
    assertthat::is.number(presence_penalty) && value_between(presence_penalty, -2, 2),
    assertthat::is.count(best_of) && best_of >= n,
    is.null(logit_bias) || is.list(logit_bias),
    assertthat::is.string(user) || is.null(user),
    assertthat::is.string(openai_api_key),
    is.null(openai_organization) || assertthat::is.string(openai_organization),
    assertthat::is.count(logprobs + 1) && value_between(logprobs, 0, 5) || is.null(logprobs)
  )
...
query_openai_api <- function(body,
  ...
  cli::cli_text("Status code: {httr::status_code(response)}")
